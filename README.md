...
```json
{
  ...
  "COLLEGE_NAME": "Your IIT Indore"
}
```

**Important**: 
- Replace `gsk_your_actual_api_key_here` with your actual Groq API key
- Replace `Your IIT Indore` with your college's actual name
- **Never commit `config.json` to version control** - add it to `.gitignore`

### Step 3: Update IIT Indore Throughout Code

The codebase contains placeholder text `"IIT Indore"` that should be replaced with your actual IIT Indore. Key files:
...
```
Find:    "IIT Indore"
Replace: "Your Actual IIT Indore"
```
